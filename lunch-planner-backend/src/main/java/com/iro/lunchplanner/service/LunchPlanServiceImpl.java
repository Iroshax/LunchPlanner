...
public class LunchPlanServiceImpl implements LunchPlanService{
    ...
    public LunchPlanServiceImpl(SessionRepository sessionRepository, LunchSessionCache lunchSessionCache) {
    ...
    private RestaurantDto selectRandomRestaurant() {

        if(!lunchSessionCache.isEmpty()){
            return lunchSessionCache.getRandomEntry().getValue();
        }else{
            throw new LunchPlanException("The open session has no restaurants.");
        }
    }
