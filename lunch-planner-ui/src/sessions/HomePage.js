...
function handleButtonClick (e) { 
    if(e === "start"){
        axios.post('http://localhost:8087/api/lunchPlanner/session/initiate',getUser())
        .then(response => {setsessionStatus("Open");console.log(response.data);toast.info('Session started successfully');})
        .catch(error =>  toast.error(` ${error.response.data.Message}`));
    }else{
         axios.post('http://localhost:8087/api/lunchPlanner/session/end',getUser())
  ...
  return (
    ...
    <div className='container'>&nbsp;
        <ToastContainer position="top-center" toaster-options="{'position-class':'toast-top-full-width'}"/>
